(function ($) {
    var hotelRoomForm = new Vue({
        ...
        computed: {
            total_price: function () {
                ...
                if (me.start_date !== "" && me.total_rooms > 0) {
                    ...
                    total_price += this.additionalPrices();
        ...
        mounted() {
            ...
            setTimeout(() => {
                var MoveDiv = document.querySelector('#hotel-rooms-form')
        ...
        methods: {
            ...
            addMealsDescription: function(event)
            {
                const selectedOption = event.target.selectedOptions[0];
                const description = selectedOption.getAttribute('data-description');
                if (!this.additional_details.meals) {
                    this.$set(this.additional_details, 'meals', []);
                }

                // Find the next index in the meals array
                const index = this.additional_details.meals.length;

                // Use $set to add the description at the next index
                this.$set(this.additional_details.meals, index, description);
                // this.additional_details.meals.push(description);
            },
            addChildrenDescription: function (event) {
                const selectedOption = event.target.selectedOptions[0];
                const description = selectedOption.getAttribute('data-description');
                if (!this.additional_details.children) {
                    this.$set(this.additional_details, 'children', []);
                }

                // Find the next index in the children array
                const index = this.additional_details.children.length;

                // Use $set to add the description at the next index
                this.$set(this.additional_details.children, index, description);
                // this.additional_details.children.push(description);
            },
            additionalPrices: function () {
                let startDate = new Date(this.start_date).getTime();
                let endDate = new Date(this.end_date).getTime();
                let duration_in_day = moment(endDate).diff(moment(startDate), 'days');

                let total = 0;
                let total_childrenPrices = 0;

                if (this.childrenPrices.length > 0)
                    total_childrenPrices += this.childrenPrices.reduce((acc, el) => acc + el, 0);

                if (this.additional_bed != "false") {
                    total += this.additional_bed * duration_in_day;
                    this.additional_details.additional_bed = this.additional_bed;
                }

                if (total_childrenPrices != 0) {
                    total += total_childrenPrices * duration_in_day;
                }

                if (this.meals != 0) {
                    total += (parseInt(this.children) + parseInt(this.adults)) * this.meals * duration_in_day;
                }

                if (this.freecancelation != 0) {
                    this.additional_details.freecancelation = true;
                    total += this.freecancelation;
                }
                return total;
            },
            doSubmit: function (e) {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        children: this.children,
                        childrenPrices: 0,
                        suplimentaryBed: this.additionalPrices(),
